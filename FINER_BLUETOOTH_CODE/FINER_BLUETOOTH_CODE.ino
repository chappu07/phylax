...
void setup() {
  ...
  SerialBT.begin("ESP32_BT_Test"); // BlueESP32_BT_Ttooth device name
  Serial.println("Bluetooth Started! Pair with est");
}
...
void loop() {
  ...
  delay(10); // A small delay in the main loop
    ...
    // If data received from Bluetooth, print it to Serial Monitor
...
else if(incoming=='0')
{
  ...
  // If data typed in Serial Monitor, send it via Bluetooth
  if (Serial.available()) {
